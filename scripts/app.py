...
def fmt_step2(text_hits, image_hits):
    lines = [f"**Hybrid Retrieval** — {len(text_hits)} text + {len(image_hits)} image chunks\n"]
...
def run_pipeline(query, mode, uploaded_image):
    ...
    if uploaded_image is not None:
        gallery.append((uploaded_image,"📤 Your uploaded image"))
    for h in image_hits:
    ...
    try:
        extra=[]
        if uploaded_image is not None:
            import io,numpy as np,tempfile
            from PIL import Image as PI
            pil=PI.fromarray(uploaded_image.astype("uint8")).convert("RGB")
            tmp=tempfile.NamedTemporaryFile(suffix=".jpg",delete=False)
            pil.save(tmp.name,"JPEG"); extra=[tmp.name]
        if mode=="Multimodal":
            ut,ip=build_multimodal_prompt(query,text_hits,image_hits)
            if extra: ut+="\n\nUser also uploaded a financial image — examine it."
            ans=generate_with_images(SYSTEM_PROMPT,ut,extra+ip,"" if extra else str(IMAGES_ROOT),1024,0.1)
        else:
...
SAMPLES=["What is the long-term debt to total liabilities for COSTCO in FY2021?",
         ...
         "What is the revenue for AMAZON in 2017?"]
...
with gr.Blocks(title="Validation-Aware Multimodal RAG ",theme=gr.themes.Soft()) as app:
    ...
    with gr.Row():
        with gr.Column(scale=3):
            qbox=gr.Textbox(label="Enter the question",placeholder="e.g. What is the long-term debt to total liabilities for COSTCO in FY2021?",lines=2)
    ...
    with gr.Accordion("📎 Upload image for visual QA",open=False):
        img_up=gr.Image(label="Upload financial chart/table",type="numpy",height=180,sources=["upload","clipboard"])
    ...
    with gr.Row():
        ...
        with gr.Column():
            gr.Markdown("### Step 2 — Hybrid Retrieval")
    ...
    gr.Markdown("---\n## 💬 Final Answer")
    ans=gr.Textbox(label="Qwen2.5-VL Answer",lines=7,interactive=False,placeholder="Answer with citations appears here...")
